...
class TapActionWrapper(base_wrapper.BaseWrapper):
    ...
    def __init__(self, env: dm_env.Environment, num_frames: int = 5, touch_only: bool = False):
        super().__init__(env)
    ...
    def _process_action(self, action: dict[str, np.ndarray]) -> Sequence[dict[str, np.ndarray]]:
        if self._touch_only:
            assert action["action_type"] == 0
            touch_action = action.copy()
            touch_action["action_type"] = np.array(action_type.ActionType.TOUCH).astype(
    ...
    def step(self, action: dict[str, np.ndarray]) -> dm_env.TimeStep:
        ...
        return dm_env.TimeStep(
            step_type=step_type, reward=total_reward, discount=discount, observation=observation
        )

    def action_spec(self) -> dict[str, dm_env.specs.Array]:
        if self._touch_only:
            return {
                "action_type": dm_env.specs.DiscreteArray(num_values=1, name="action_type"),
                "touch_position": self._env.action_spec()["touch_position"],
            }
        else:
