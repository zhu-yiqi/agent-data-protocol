...
import dm_env
import numpy as np
from android_env.components import action_type, utils
...
class LastActionWrapper(base_wrapper.BaseWrapper):
    ...
    def __init__(self, env: dm_env.Environment, concat_to_pixels: bool = True):
    ...
    def _process_observation(self, observation: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    def _get_last_action_layer(self, pixels: np.ndarray) -> np.ndarray:
        ...
        if (
            "action_type" in last_action
            and last_action["action_type"] == action_type.ActionType.TOUCH
        ):
    ...
    def observation_spec(self) -> dict[str, specs.Array]:
        ...
        if self._concat_to_pixels:
            ...
            parent_spec.update(
                {
                    "last_action": specs.BoundedArray(
