...
class ImageRescaleWrapper(base_wrapper.BaseWrapper):
    ...
    def _process_timestep(self, timestep: dm_env.TimeStep) -> dm_env.TimeStep:
        ...
        processed_observation["pixels"] = self._process_pixels(observation["pixels"])
    ...
    def _process_pixels(self, raw_observation: np.ndarray) -> np.ndarray:
        ...
        new_shape = np.array(
            self._zoom_factors[0:2] * np.array(raw_observation.shape[0:2]), dtype=np.int32
        )[::-1]
        if self._grayscale:
    ...
    def _resize_image_array(
        self, grayscale_or_rbg_array: np.ndarray, new_shape: Sequence[int]
    ) -> np.ndarray:
        ...
        resized_array = np.array(
            Image.fromarray(grayscale_or_rbg_array.astype("uint8")).resize(new_shape)
        )
    ...
    def observation_spec(self) -> dict[str, specs.Array]:
        ...
        out_shape = np.multiply(parent_spec["pixels"].shape, self._zoom_factors).astype(np.int32)
        if self._grayscale:
