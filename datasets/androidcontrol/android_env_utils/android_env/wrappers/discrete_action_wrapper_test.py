...
def _valid_shape(action):
    ...
    assert not action["action_type"].shape, "action: %r, shape: %r" % (
        action["action_type"],
        action["action_type"].shape,
    )
    assert action["touch_position"].shape == (2,), "action: %r, shape: %r" % (
        action["touch_position"],
        action["touch_position"].shape,
    )
...
class DiscreteActionWrapperTest(absltest.TestCase):
    def setUp(self):
        ...
        self._base_action_spec = {
            ...
            "touch_position": _make_array_spec(shape=(2,), dtype=np.float32, name="touch_position"),
        }
    ...
    def test_step_no_noise(self):
        ...
        width = 3
        wrapped_env = discrete_action_wrapper.DiscreteActionWrapper(
            self.base_env, action_grid=(height, width), noise=0.0, redundant_actions=True
        )
        self.assertEqual(height * width * self._num_action_types, wrapped_env.num_actions)

        vertical_half_step = 1.0 / float(height) / 2.0
        horizontal_half_step = 1.0 / float(width) / 2.0
        ...
        def get_verifier(expected_action_type, lower_x, lower_y):
            def verifier(x):
                ...
                if lower_y:
                    self.assertAlmostEqual(vertical_half_step, x["touch_position"][1], delta=delta)
                else:
    ...
    def test_step_redundant_actions_invalid_action_id(self):
        wrapped_env = discrete_action_wrapper.DiscreteActionWrapper(
            self.base_env, action_grid=(4, 3), noise=0.0, redundant_actions=True
        )
    ...
    def test_step_no_noise_no_redudant_actions(self):
        ...
        width = 3
        wrapped_env = discrete_action_wrapper.DiscreteActionWrapper(
            self.base_env, action_grid=(height, width), noise=0.0, redundant_actions=False
        )
        self.assertEqual(height * width + (self._num_action_types - 1), wrapped_env.num_actions)
    ...
    def test_step_no_redundant_actions_invalid_action_id(self):
        wrapped_env = discrete_action_wrapper.DiscreteActionWrapper(
            self.base_env, action_grid=(4, 3), noise=0.0, redundant_actions=False
        )
        with self.assertRaises(AssertionError):
    ...
    def test_step_with_noise(self):
        ...
        vertical_grid_step = 1.0 / float(height)
        horizontal_grid_step = 1.0 / float(width)
    ...
    def test_parent_spec_type(self):
        base_action_spec = {
            ...
            "touch_position": _make_array_spec(shape=(2,), dtype=np.float64, name="touch_position"),
        }
        base_env = mock.create_autospec(env_interface.AndroidEnvInterface)
        base_env.action_spec.return_value = base_action_spec

        wrapped_env = discrete_action_wrapper.DiscreteActionWrapper(base_env, noise=0.0)
    ...
    def test_observation_spec(self):
        wrapped_env = discrete_action_wrapper.DiscreteActionWrapper(self.base_env)
    ...
    def test_assert_base_env_action_spec_too_long(self):
        self.base_env.action_spec.return_value = {
            ...
            "touch_position": _make_array_spec(shape=(2,), dtype=np.float32, name="touch_position"),
            "too_long": _make_array_spec(shape=(1,), dtype=np.float32, name="too_long"),
        }
        with self.assertRaises(AssertionError):
            _ = discrete_action_wrapper.DiscreteActionWrapper(self.base_env)
    ...
    def test_assert_base_env_action_spec_wrong_shapes(self):
        self.base_env.action_spec.return_value = {
            "action_type": _make_array_spec(shape=(2,), dtype=np.float32, name="action_type"),
            "touch_position": _make_array_spec(shape=(1,), dtype=np.float32, name="touch_position"),
        }
        with self.assertRaises(AssertionError):
            _ = discrete_action_wrapper.DiscreteActionWrapper(self.base_env)
