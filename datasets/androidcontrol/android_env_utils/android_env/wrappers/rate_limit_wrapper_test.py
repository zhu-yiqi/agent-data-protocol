...
class RateLimitWrapperTest(parameterized.TestCase):
    ...
    def test_enabled_sleep_type_before(self, mock_sleep):
        ...
        wrapper = rate_limit_wrapper.RateLimitWrapper(
            env, rate=1 / 33.33, sleep_type=rate_limit_wrapper.RateLimitWrapper.SleepType.BEFORE
        )
        ...
        def _step_fn(action):
            self.assertEqual(
                action["action_type"], np.array(action_type.ActionType.LIFT, dtype=np.uint8)
            )
    ...
    def test_enabled_sleep_type_after(self, mock_sleep):
        ...
        env = _get_base_env()
        wrapper = rate_limit_wrapper.RateLimitWrapper(
            env, rate=1 / 33.33, sleep_type=rate_limit_wrapper.RateLimitWrapper.SleepType.AFTER
        )
        _ = wrapper.reset()
        mock_sleep.assert_not_called()  # It should never sleep during reset().
    ...
    def test_enabled_sleep_type_after_with_repeat(self, mock_sleep):
        ...
        def _step_fn(action):
            ...
            if len(_step_fn.timestamps) % 2 == 0:
                ...
                self.assertEqual(
                    action["action_type"], np.array(action_type.ActionType.REPEAT, dtype=np.uint8)
                )
            _step_fn.timestamps.append(time.time())
