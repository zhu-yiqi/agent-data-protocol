...
class FlatInterfaceWrapper(base_wrapper.BaseWrapper):
    ...
    def _process_timestep(self, timestep: dm_env.TimeStep) -> dm_env.TimeStep:
        if self._flat_observations:
            ...
            pixels = pixels if self._keep_action_layer else _extract_screen_pixels(pixels)
            return dm_env.TimeStep(
                step_type=step_type, reward=reward, discount=discount, observation=pixels
            )
        else:
            return timestep
    ...
    def action_spec(
        ...
        specs.BoundedArray | dict[str, specs.Array]
    ):  # pytype: disable=signature-mismatch  # overriding-return-type-checks
        if self._flat_actions:
