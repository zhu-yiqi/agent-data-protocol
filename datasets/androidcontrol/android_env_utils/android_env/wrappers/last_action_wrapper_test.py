...
def _simple_spec():
    return specs.BoundedArray(
        shape=np.array([120, 80, 3]), dtype=np.uint8, name="pixels", minimum=0, maximum=255
    )
...
class LastActionWrapperTest(absltest.TestCase):
    def test_concat_to_pixels(self):
        ...
        wrapper = last_action_wrapper.LastActionWrapper(fake_env, concat_to_pixels=True)
    ...
    def test_no_concat_to_pixels(self):
        ...
        wrapper = last_action_wrapper.LastActionWrapper(fake_env, concat_to_pixels=False)
        self.assertIsNotNone(wrapper)
