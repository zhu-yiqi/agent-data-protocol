...
class FlatInterfaceWrapperTest(absltest.TestCase):
    def setUp(self):
        ...
        self.base_observation_spec = {
            "pixels": _make_array_spec(shape=self.int_obs_shape, dtype=np.uint8, name="pixels"),
            "obs1": _make_array_spec(shape=self.float_obs_shape, dtype=np.float32, name="obs1"),
        }
        # Expected.
        ...
        self.base_timestep = _make_timestep({"pixels": self.image_obs, "obs1": self.other_obs})
    ...
    def test_reset_no_action_layer(self):
        ...
        self.base_env.reset.assert_called_once()
        self.assertEqual(
            self.expected_timestep_no_action_layer.observation.tolist(), ts.observation.tolist()
        )
    ...
    def test_observation_spec_no_action_layer(self):
        ...
        observation_spec = wrapped_env.observation_spec()
        self.base_env.observation_spec.assert_called_once()
        self.assertEqual(self.expected_observation_spec_no_action_layer, observation_spec)
