...
from android_env.proto.a11y import a11y_pb2, a11y_pb2_grpc, android_accessibility_forest_pb2
from android_env.wrappers import a11y_grpc_wrapper


def empty_forest() -> android_accessibility_forest_pb2.AndroidAccessibilityForest:
    return android_accessibility_forest_pb2.AndroidAccessibilityForest()


def one_empty_window_forest() -> android_accessibility_forest_pb2.AndroidAccessibilityForest:
    forest = android_accessibility_forest_pb2.AndroidAccessibilityForest()
    _ = forest.windows.add()
    return forest


def one_window_one_node_forest() -> android_accessibility_forest_pb2.AndroidAccessibilityForest:
    forest = android_accessibility_forest_pb2.AndroidAccessibilityForest()
...
def one_window_two_nodes_forest() -> android_accessibility_forest_pb2.AndroidAccessibilityForest:
    forest = android_accessibility_forest_pb2.AndroidAccessibilityForest()
    window = forest.windows.add()
    node = window.tree.nodes.add()
    node.class_name = "bar"
...
def three_windows_forest() -> android_accessibility_forest_pb2.AndroidAccessibilityForest:
    forest = android_accessibility_forest_pb2.AndroidAccessibilityForest()
    _ = forest.windows.add()
    window = forest.windows.add()
    node = window.tree.nodes.add()
...
class A11yGrpcWrapperTest(parameterized.TestCase):
    def test_server(self):
        base_env = mock.create_autospec(env_interface.AndroidEnvInterface, instance=True)
        ...
        with grpc.secure_channel(f"[::]:{wrapped_env.get_port()}", channel_creds) as channel:
    ...
    @mock.patch.object(a11y_pb2_grpc, "add_A11yServiceServicer_to_server", autospec=True)
    ...
    def test_fetch_task_extras(
        ...
        for event in received_events:
            wrapped_env._servicer.SendEvent(a11y_pb2.EventRequest(event=event), mock_context)
    ...
    def test_fetch_task_extras_enable_networking_twice(
        ...
        self.assertEqual(extras["accessibility_tree"][0], one_window_one_node_forest())
    ...
    def test_task_extras_rasises_with_a11y_info_exception(
        ...
        base_env.step.return_value = dm_env.transition(observation={"dummy": 42}, reward=0.0)
        ...
        self.assertRaises(a11y_grpc_wrapper.EnableNetworkingError, wrapped_env.step, {})
    ...
    def test_configure_grpc(
        ...
        base_env.execute_adb_call.return_value = _ok_response()
        wrapped_env = a11y_grpc_wrapper.A11yGrpcWrapper(base_env)
        with mock.patch.object(wrapped_env, "_configure_grpc") as mock_configure_grpc:
            wrapped_env.reset()
            mock_configure_grpc.assert_called_once()

    @mock.patch.object(a11y_pb2_grpc, "add_A11yServiceServicer_to_server", autospec=True)
    @mock.patch.object(grpc, "server", autospec=True)
    def test_task_extras_raises_before_reset(self, unused_mock_server, unused_mock_add_servicer):
        base_env = mock.create_autospec(env_interface.AndroidEnvInterface, instance=True)
        base_env.stats.return_value = {"relaunch_count": 0}
        base_env.execute_adb_call.return_value = _ok_response()
        wrapped_env = a11y_grpc_wrapper.A11yGrpcWrapper(base_env)
    ...
    @mock.patch.object(a11y_pb2_grpc, "add_A11yServiceServicer_to_server", autospec=True)
    @mock.patch.object(grpc, "server", autospec=True)
    def test_extras_accumulate_between_steps(self, mock_server, mock_add_servicer):
        del mock_server, mock_add_servicer
        base_env = mock.create_autospec(env_interface.AndroidEnvInterface, instance=True)
        base_env.stats.return_value = {"relaunch_count": 0}
        base_env.execute_adb_call.return_value = _ok_response()
        base_env.reset.return_value = dm_env.restart(observation={"dummy": 42})
        base_env.step.return_value = dm_env.transition(observation={"dummy": 42}, reward=0.0)
        wrapped_env = a11y_grpc_wrapper.A11yGrpcWrapper(base_env, add_latest_a11y_info_to_obs=True)
        with mock.patch.object(wrapped_env, "_fetch_task_extras"):
            ...
            self.assertIn("a11y_forest", timestep.observation)
            self.assertEqual(timestep.observation["a11y_forest"], one_window_two_nodes_forest())
            timestep = wrapped_env.step({})
            self.assertIn("a11y_forest", timestep.observation)
            self.assertEqual(timestep.observation["a11y_forest"], one_window_two_nodes_forest())
            wrapped_env._fetch_task_extras.return_value = {
        ...
        expected_task_extras_latest = {
            ...
            "accessibility_tree": np.array([one_window_two_nodes_forest()], dtype=object),
        }
        task_extras = wrapped_env.task_extras(latest_only=False)
        np.testing.assert_equal(task_extras["full_event"], expected_task_extras["full_event"])
    ...
    def test_task_extras_same_between_calls(self, mock_server, mock_add_servicer):
        ...
        expected_task_extras = {
            "full_event": np.array(empty_dict(), ndmin=1, dtype=object),
            "accessibility_tree": np.array(one_window_two_nodes_forest(), ndmin=1, dtype=object),
        }
        with mock.patch.object(wrapped_env, "_fetch_task_extras"):
            wrapped_env._fetch_task_extras.return_value = expected_task_extras
            wrapped_env.step({})
    ...
    @mock.patch.object(a11y_pb2_grpc, "add_A11yServiceServicer_to_server", autospec=True)
    @mock.patch.object(grpc, "server", autospec=True)
    def test_task_extras_clear_if_called_between_step(self, mock_server, mock_add_servicer):
        del mock_server, mock_add_servicer
        base_env = mock.create_autospec(env_interface.AndroidEnvInterface, instance=True)
        base_env.stats.return_value = {"relaunch_count": 0}
        base_env.execute_adb_call.return_value = _ok_response()
