...
def _make_bounded_array_spec(shape, dtype=np.float32, name=None, maximum=1.0, minimum=0.0):
...
class FloatPixelsWrapperTest(absltest.TestCase):
    def setUp(self):
        ...
        base_pixel_spec = _make_array_spec(shape=self.pixels_shape, dtype=np.uint8, name="pixels")
        self.other_obs_spec = _make_array_spec(shape=(1,), dtype=np.float32, name="other_obs")
        base_observation_spec = {"pixels": base_pixel_spec, "other_obs": self.other_obs_spec}
    ...
    def test_float_pixels_wrapper_spec(self):
        expected_pixel_spec = _make_bounded_array_spec(
            shape=self.pixels_shape, dtype=np.float32, name="pixels", minimum=0.0, maximum=1.0
        )

        wrapped_env = float_pixels_wrapper.FloatPixelsWrapper(self.base_env)

        self.assertLen(wrapped_env.observation_spec(), 2)
        self.assertEqual(expected_pixel_spec, wrapped_env.observation_spec()["pixels"])
        self.assertEqual(self.other_obs_spec, wrapped_env.observation_spec()["other_obs"])
    ...
    def test_float_pixels_wrapper_step(self):
        ...
        self.assertEqual(self.base_timestep.observation["other_obs"], ts.observation["other_obs"])
        expected_pixel_value = 1.0 / 255.0  # original values are unit8
        expected_pixels = np.ones(self.pixels_shape, dtype=np.float32) * expected_pixel_value
    ...
    def test_float_pixels_wrapper_already_float(self):
        base_pixel_spec = _make_array_spec(shape=self.pixels_shape, dtype=np.float64, name="pixels")
        base_observation_spec = {"pixels": base_pixel_spec, "other_obs": self.other_obs_spec}
        ...
        self.assertEqual(base_env.observation_spec(), wrapped_env.observation_spec())
