...
class UtilsTest(parameterized.TestCase):
    ...
    def test_touch_position_to_pixel_position(self, touch_pos, width_height, pixel_pos):
        self.assertEqual(
            utils.touch_position_to_pixel_position(np.array(touch_pos), width_height), pixel_pos
        )
    ...
    def test_convert_int_to_float_bounded_array(self):
        ...
        np.testing.assert_equal(
            np.array([2.0 / 5.0, 1.0 / 4.0, 0.0, 0.5], dtype=np.float64), float_data
        )
    ...
    def test_convert_int_to_float_bounded_array_broadcast(self):
        ...
        np.testing.assert_equal(np.array([0.0, 0.5, 1.0], dtype=np.float32), float_data)
    ...
    def test_convert_int_to_float_no_bounds(self):
        ...
        float_data = utils.convert_int_to_float(data, spec, np.float32)
        np.testing.assert_equal(np.array([0.0, 128.0 / 255.0, 1.0], dtype=np.float32), float_data)
