...
class _DumpsysNode:
    ...
    def find_child(self, predicate: Callable[[Self], bool], max_levels: int = 0) -> Self | None:
...
def matches_path(
    ...
    view_hierarchy = root.find_child(lambda x: x.data.startswith("View Hierarchy"), max_levels)
    if view_hierarchy is None:
        logging.error(
            "view_hierarchy is None. Dumpsys activity output: %s. tree: %r",
            str(dumpsys_activity_output),
            root.print_tree(),
        )
    ...
    for i, regex in enumerate(expected_view_hierarchy_path):
        ...
        if child is None:
            logging.error(
                "Mismatched regex (%i, %s). current_node: %s", i, regex.pattern, current_node
            )
...
class AppScreenChecker:
    ...
    def matches_current_app_screen(self) -> enum.IntEnum:
        ...
        response = self._adb_call_parser.parse(
            adb_pb2.AdbRequest(get_current_activity=adb_pb2.AdbRequest.GetCurrentActivity())
        )
        ...
        if current_activity != self._expected_activity:
            logging.error(
                "current_activity: %s,  expected_activity: %s",
                current_activity,
    ...
    def wait_for_app_screen(self, timeout_sec: float) -> float:
        ...
        while time.time() - start_time < timeout_sec:
            if self.matches_current_app_screen() == AppScreenChecker.Outcome.SUCCESS:
                ...
                logging.info(
                    "Successfully waited for app screen in %r seconds: [%r]",
                    wait_time,
        ...
        wait_time = time.time() - start_time
        logging.error(
            "Failed to wait for app screen in %r seconds: [%r].",
            wait_time,
            self._expected_app_screen,
        )
