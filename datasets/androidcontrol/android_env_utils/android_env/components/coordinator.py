...
from android_env.components import adb_call_parser, config_classes, errors, specs, utils
...
class Coordinator:
    ...
    def observation_spec(self) -> dict[str, dm_env.specs.Array]:
        return specs.base_observation_spec(height=self._screen_size[0], width=self._screen_size[1])
    ...
    def _update_device_orientation(self) -> None:
        ...
        orientation_response = self._adb_call_parser.parse(
            adb_pb2.AdbRequest(get_orientation=adb_pb2.AdbRequest.GetOrientationRequest())
        )
    ...
    def _gather_simulator_signals(self) -> dict[str, np.ndarray]:
        ...
        now = time.time()
        timestamp_delta = (
            0 if self._latest_observation_time == 0 else (now - self._latest_observation_time) * 1e6
        )
        ...
        try:
            match action["action_type"]:
                ...
                case action_type_lib.ActionType.KEYDOWN:
                    self._simulator.send_key(action["keycode"].item(0), event_type="keydown")
    ...
    def _split_touch_action(self, action: dict[str, np.ndarray]) -> list[dict[str, np.ndarray]]:
...
class InteractionThread(threading.Thread):
    ...
    def __init__(self, simulator: base_simulator.BaseSimulator, interaction_rate_sec: float):
        super().__init__()
        self._simulator = simulator
