...
from android_env.components import config_classes, errors
...
class AdbController:
    ...
    def __init__(self, config: config_classes.AdbControllerConfig):
        ...
        self._os_env_vars.update({"HOME": os.path.expandvars(self._os_env_vars.get("HOME", ""))})
    ...
    def _restart_server(self, timeout: float | None = None):
        ...
        self.execute_command(["kill-server"], timeout=timeout, device_specific=False)
        time.sleep(0.2)
        cmd_output = self.execute_command(["start-server"], timeout=timeout, device_specific=False)
    ...
    def execute_command(
        ...
        while n_tries <= n_retries:
            try:
                ...
                logging.exception(
                    "Failed to execute ADB command (try %r of 3): [%s]", n_tries, command_str
                )
