...
class EmulatorSimulator(base_simulator.BaseSimulator):
    ...
    def __init__(self, config: config_classes.EmulatorConfig):
        ...
        if _is_existing_emulator_provided(self._config.emulator_launcher):
            ...
            self._config.emulator_launcher.emulator_console_port = portpicker.pick_unused_port()
            self._config.emulator_launcher.grpc_port = _pick_emulator_grpc_port()

        self._channel = None
        self._emulator_stub: emulator_controller_pb2_grpc.EmulatorControllerStub | None = None
        ...
        if self._existing_emulator_provided:
            ...
            logging.info("emulator_launcher config: %r", self._config.emulator_launcher)
            ...
            self._logfile_path = self._config.logfile_path or self._launcher.logfile_path()
    ...
    def _launch_impl(self) -> None:
        ...
        if self._launcher is not None:
            ...
            if (
                ...
                and self._num_launch_attempts > self._config.launch_n_times_without_reboot
            ):
                self._shutdown_emulator()
                # Subsequent attempts cause the emulator files to be reinstalled.
                if self._num_launch_attempts > self._config.launch_n_times_without_reinstall:
    ...
    def load_state(self, request: state_pb2.LoadStateRequest) -> state_pb2.LoadStateResponse:
        ...
        if any(snapshot.snapshot_id == snapshot_name for snapshot in snapshot_list.snapshots):
            snapshot_result = self._snapshot_stub.LoadSnapshot(
                snapshot_service_pb2.SnapshotPackage(snapshot_id=snapshot_name)
            )
            if snapshot_result.success:
                return state_pb2.LoadStateResponse(status=state_pb2.LoadStateResponse.Status.OK)
            else:
            ...
            return state_pb2.LoadStateResponse(status=state_pb2.LoadStateResponse.Status.NOT_FOUND)

    def save_state(self, request: state_pb2.SaveStateRequest) -> state_pb2.SaveStateResponse:
        ...
        if snapshot_result.success:
            return state_pb2.SaveStateResponse(status=state_pb2.SaveStateResponse.Status.OK)
        else:
    ...
    def _connect_to_emulator(
        ...
        try:
            ...
            raise EmulatorBootError("Failed to connect to the emulator.") from grpc_error

        logging.info("Added gRPC channel for the Emulator on port %s", port)
        emulator_controller_stub = emulator_controller_pb2_grpc.EmulatorControllerStub(
            self._channel
        )
    ...
    def send_touch(self, touches: list[tuple[int, int, bool, int]]) -> None:
        ...
        assert self._emulator_stub is not None, "Emulator stub has not been initialized yet."
        touch_events = [
            emulator_controller_pb2.Touch(x=t[0], y=t[1], pressure=int(t[2]), identifier=t[3])
            for t in touches
        ]
        self._emulator_stub.sendTouch(emulator_controller_pb2.TouchEvent(touches=touch_events))
