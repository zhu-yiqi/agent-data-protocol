...
from android_env.components import adb_call_parser, adb_controller, config_classes
...
class EmulatorSimulatorTest(absltest.TestCase):
    def setUp(self):
        ...
        mock.patch.object(grpc.aio, "secure_channel", return_value=self._grpc_channel).start()
        mock.patch.object(grpc, "secure_channel", return_value=self._grpc_channel).start()
        mock.patch.object(
            grpc, "local_channel_credentials", return_value=self._grpc_channel
        ).start()
        self._mock_future = mock.create_autospec(grpc.Future)
        mock.patch.object(grpc, "channel_ready_future", return_value=self._mock_future).start()
        ...
        mock.patch.object(
            adb_call_parser, "AdbCallParser", autospec=True, return_value=self._adb_call_parser
        ).start()
        mock.patch.object(
            emulator_launcher, "EmulatorLauncher", return_value=self._launcher
        ).start()
    ...
    def test_logfile_path(self, mock_open, unused_mock_exists):
        ...
        simulator = emulator_simulator.EmulatorSimulator(config)
        mock_open.return_value.__enter__.return_value.read.return_value = "fake_logs".encode(
            "utf-8"
        )
    ...
    def test_launch_operation_order(self):
        ...
        # Arrange.
        call_order = []
        self._adb_controller.init_server.side_effect = lambda: call_order.append("init_server")
        self._launcher.launch_emulator_process.side_effect = lambda: call_order.append(
    ...
    def test_load_state(self):
        ...
        with mock.patch.object(
            ...
            mock_snapshot_stub.LoadSnapshot.return_value = snapshot_service_pb2.SnapshotPackage(
                success=True
            )
            load_response = simulator.load_state(
                request=state_pb2.LoadStateRequest(args={"snapshot_name": "snapshot_name_foo"})
            )
            self.assertEqual(load_response.status, state_pb2.LoadStateResponse.Status.OK)
            load_response = simulator.load_state(
                request=state_pb2.LoadStateRequest(args={"snapshot_name": "snapshot_name_baz"})
            )
            self.assertEqual(load_response.status, state_pb2.LoadStateResponse.Status.NOT_FOUND)
            mock_snapshot_stub.LoadSnapshot.return_value = snapshot_service_pb2.SnapshotPackage(
                success=False, err=b"error"
            )
            load_response = simulator.load_state(
                request=state_pb2.LoadStateRequest(args={"snapshot_name": "snapshot_name_bar"})
            )
            self.assertEqual(load_response.status, state_pb2.LoadStateResponse.Status.ERROR)
    ...
    def test_save_state(self):
        ...
        with mock.patch.object(
            ...
            mock_snapshot_stub.SaveSnapshot.return_value = snapshot_service_pb2.SnapshotPackage(
                success=True
            )
            save_response = simulator.save_state(
                request=state_pb2.SaveStateRequest(args={"snapshot_name": "snapshot_name_foo"})
            )
            self.assertEqual(save_response.status, state_pb2.SaveStateResponse.Status.OK)
            mock_snapshot_stub.SaveSnapshot.return_value = snapshot_service_pb2.SnapshotPackage(
                success=False, err=b"error"
            )
            save_response = simulator.save_state(
                request=state_pb2.SaveStateRequest(args={"snapshot_name": "snapshot_name_bar"})
            )
            self.assertEqual(save_response.status, state_pb2.SaveStateResponse.Status.ERROR)
    ...
    def test_send_touch(self):
        ...
        simulator._emulator_stub.sendTouch.assert_has_calls(
            [
                mock.call(
                    emulator_controller_pb2.TouchEvent(
                        touches=[
                            {"x": 123, "y": 456, "pressure": 1},
                            {"x": 135, "y": 246, "pressure": 1, "identifier": 1},
                        ]
                    )
                ),
                mock.call(
                    emulator_controller_pb2.TouchEvent(
                        touches=[
                            {"x": 1, "y": 2, "pressure": 1},
                            {"x": 3, "y": 4, "pressure": 1, "identifier": 1},
                        ]
                    )
                ),
                mock.call(
                    emulator_controller_pb2.TouchEvent(
                        touches=[
                            {"x": 321, "y": 654, "pressure": 0},
                            {"x": 531, "y": 642, "pressure": 0, "identifier": 1},
                        ]
                    )
                ),
            ]
