...
class EmulatorLauncher:
    ...
    def launch_emulator_process(self) -> None:
        ...
        extra_env_vars = {
            ...
            "ANDROID_ADB_SERVER_PORT": str(self._adb_controller_config.adb_server_port),
            "LD_LIBRARY_PATH": ld_library_path,
            "QT_XKB_CONFIG_ROOT": str(self._config.emulator_path[:-8] + "qt_config/"),
            "ANDROID_EMU_ENABLE_CRASH_REPORTING": "1",
            "SHOW_PERF_STATS": str(1 if self._config.show_perf_stats else 0),
        }
        logging.info("extra_env_vars: %s", " ".join(f"{k}={v}" for k, v in extra_env_vars.items()))
        env_vars = dict(os.environ).copy()
        env_vars.update(extra_env_vars)

        # Compile command.
        grpc_port = ["-grpc", str(self._config.grpc_port)] if self._config.grpc_port >= 0 else []
        run_headless = ["-no-skin", "-no-window"] if self._config.run_headless else []
        ports = [
            "-ports",
        ...
        network_args = restrict_network_args if self._config.restrict_network else []
        command = (
            [
        ...
        self._emulator = subprocess.Popen(
            command, env=env_vars, stdout=self._emulator_output, stderr=self._emulator_output
        )
