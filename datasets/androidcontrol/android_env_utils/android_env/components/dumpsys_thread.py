...
class DumpsysThread:
    ...
    def check_user_exited(self, timeout: float | None = None) -> bool:
        ...
        if self._check_frequency <= 0 or self._main_loop_counter < self._check_frequency:
            return False
        self._main_loop_counter = 0
    ...
    def _check_impl(self) -> bool:
        ...
        if outcome == _Outcome.FAILED_ACTIVITY_EXTRACTION:
            ...
            logging.info(
                "self._num_failed_activity_extraction: %s", self._num_failed_activity_extraction
            )
            if self._num_failed_activity_extraction >= self._max_failed_activity_extraction:
        ...
        if outcome == _Outcome.SUCCESS or outcome == _Outcome.EMPTY_EXPECTED_ACTIVITY:
            return False
