...
from android_env.components import adb_call_parser, app_screen_checker, errors
from android_env.proto import adb_pb2, task_pb2


def _flatten_tree(tree: app_screen_checker._DumpsysNode, flat_tree: list[str], indent: int = 2):
...
"""
        expected_path = ["^A$", "B$"]
        expected_view_hierarchy_path = [re.compile(regex) for regex in expected_path]
        self.assertFalse(
            app_screen_checker.matches_path(dumpsys_output, expected_view_hierarchy_path)
        )
...
"""
    ...
    def test_wait_for_app_screen_zero_timeout(self):
        ...
        screen_checker = app_screen_checker.AppScreenChecker(
            adb_call_parser=call_parser, expected_app_screen=app_screen
        )
        # With a zero timeout, the method should never be able to wait for the
        # screen to pop up and an exception should be raised.
        self.assertRaises(
            errors.WaitForAppScreenError, screen_checker.wait_for_app_screen, timeout_sec=0.0
        )
    ...
    def test_wait_for_app_screen_successful(self):
        ...
        screen_checker = app_screen_checker.AppScreenChecker(call_parser, app_screen)
        timeout = 1.0
