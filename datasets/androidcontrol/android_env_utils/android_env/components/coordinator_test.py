...
class CoordinatorTest(parameterized.TestCase):
    ...
    def test_relaunch_simulator(self, unused_mock_sleep):
        ...
        self.assertEqual(self._coordinator.stats()["relaunch_count"], relaunch_count + 1)
    ...
    def test_process_action(self, unused_mock_sleep):
        ...
        np.testing.assert_equal(obs["orientation"], np.array([0, 0, 0, 0], dtype=np.uint8))
    ...
    def test_process_action_error_async(self, unused_mock_sleep):
        mock_interaction_thread = mock.create_autospec(coordinator_lib.InteractionThread)
        with mock.patch.object(
            ...
            coordinator = coordinator_lib.Coordinator(
                simulator=self._simulator,
                task_manager=self._task_manager,
                config=config_classes.CoordinatorConfig(num_fingers=1, interaction_rate_sec=0.016),
            )
    ...
    def test_async_step_faster_than_screenshot(self):
        ...
        coordinator = coordinator_lib.Coordinator(
            simulator=self._simulator,
            task_manager=self._task_manager,
            config=config_classes.CoordinatorConfig(num_fingers=1, interaction_rate_sec=0.5),
        )
        ...
        np.testing.assert_almost_equal(ts2.observation["pixels"], ts1.observation["pixels"])
        coordinator.close()
    ...
    def test_async_step_slower_than_screenshot(self):
        ...
        coordinator = coordinator_lib.Coordinator(
            simulator=self._simulator,
            task_manager=self._task_manager,
            config=config_classes.CoordinatorConfig(num_fingers=1, interaction_rate_sec=0.01),
        )
        ts1 = coordinator.rl_step(
            agent_action={
                "action_type": np.array(action_type.ActionType.LIFT),
    ...
    def test_execute_action_touch(self, unused_mock_sleep):
        ...
        np.testing.assert_equal(timestep.observation["pixels"], self._random_screenshot)
    ...
    def test_execute_multitouch_action(self, unused_mock_sleep):
        ...
        self._simulator.send_touch.assert_called_once_with(
            [(150, 600, True, 0), (450, 200, True, 1), (300, 400, False, 2)]
        )
    ...
    def test_max_restarts_setup_steps(self, unused_mock_sleep):
        ...
        self.assertRaises(errors.TooManyRestartsError, self._coordinator._launch_simulator)
        # The method was called three more times when attempting to relaunch.
        self.assertEqual(init_fn_call + 3, self._task_manager.setup_task.call_count)

    @mock.patch.object(time, "sleep", autospec=True)
    def test_execute_adb_call(self, unused_mock_sleep):
        call = adb_pb2.AdbRequest(force_stop=adb_pb2.AdbRequest.ForceStop(package_name="blah"))
        expected_response = adb_pb2.AdbResponse(status=adb_pb2.AdbResponse.Status.OK)
    ...
    def test_with_tmp_dir_no_tempfile_call(self, mock_gettempdir, unused_mock_sleep):
        """If passing a `tmp_dir`, `tempfile.gettempdir()` should not be called."""
        _ = coordinator_lib.Coordinator(
            simulator=self._simulator,
            task_manager=self._task_manager,
            config=config_classes.CoordinatorConfig(tmp_dir=absltest.get_default_test_tmpdir()),
        )
    ...
    def test_no_tmp_dir_calls_tempfile(self, mock_gettempdir, unused_mock_sleep):
        ...
        _ = coordinator_lib.Coordinator(simulator=self._simulator, task_manager=self._task_manager)
    ...
    @mock.patch.object(time, "sleep", autospec=True)
    def test_bar_visibility(
        self, show_navigation_bar, show_status_bar, expected_value, unused_mock_sleep
    ):
        _ = coordinator_lib.Coordinator(
            simulator=self._simulator,
            task_manager=self._task_manager,
