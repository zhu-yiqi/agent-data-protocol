...
class TaskManagerTest(absltest.TestCase):
    def setUp(self):
        ...
        mock.patch.object(
            ...
            return_value=self._setup_step_interpreter,
        ).start()
        mock.patch.object(
            dumpsys_thread, "DumpsysThread", return_value=self._dumpsys_thread
        ).start()
        mock.patch.object(logcat_thread, "LogcatThread", return_value=self._logcat_thread).start()
        mock.patch.object(log_stream, "LogStream", return_value=self._log_stream).start()
    ...
    def test_get_current_reward(self):
        ...
        task = task_pb2.Task()
        task.log_parsing_config.log_regexps.reward.extend(["^[Rr]eward: ([-+]?[0-9]*\\.?[0-9]*)$"])
    ...
    def test_reward_event(self):
        ...
        task = task_pb2.Task()
        reward_event_1 = task_pb2.LogParsingConfig.LogRegexps.RewardEvent(event="foo_1", reward=5.0)
        reward_event_2 = task_pb2.LogParsingConfig.LogRegexps.RewardEvent(
            event="foo_2", reward=-1.0
        )
        task.log_parsing_config.log_regexps.reward_event.extend([reward_event_1, reward_event_2])
        task.log_parsing_config.log_regexps.reward.extend(["^[Rr]eward: ([-+]?[0-9]*\\.?[0-9]*)$"])
        task_mgr = task_manager.TaskManager(task=task)
        self._logcat_thread.add_event_listener.side_effect = my_add_ev_listener
        adb_call_parser = mock.create_autospec(adb_call_parser_lib.AdbCallParser)
    ...
    def test_get_current_reward_via_score(self):
        ...
        task = task_pb2.Task()
        task.log_parsing_config.log_regexps.score = "^score: ([-+]?[0-9]*\\.?[0-9]*)$"
        task_mgr = task_manager.TaskManager(task=task)
        self._logcat_thread.add_event_listener.side_effect = my_add_ev_listener
        adb_call_parser = mock.create_autospec(adb_call_parser_lib.AdbCallParser)
        task_mgr.start(lambda: adb_call_parser, log_stream=self._log_stream)
    ...
    def test_get_current_extras(self):
        ...
        np.testing.assert_almost_equal([[1, 2, 3], [4, 5, 6]], extras.get("an_extra"))
        np.testing.assert_almost_equal([0.5], extras.get("another_extra"))
        np.testing.assert_almost_equal(
            [[[9, 8, 7], [6, 5, 4]]], extras.get("multi_dimension_extra")
        )
    ...
    def test_get_current_extras_json_format(self):
        ...
        # Setup the task and trigger the listener.
        task = task_pb2.Task()
        task.log_parsing_config.log_regexps.json_extra.extend(["^json_extra: (?P<json_extra>.*)$"])
        task_mgr = task_manager.TaskManager(task=task)
        self._logcat_thread.add_event_listener.side_effect = my_add_ev_listener
        adb_call_parser = mock.create_autospec(adb_call_parser_lib.AdbCallParser)
        task_mgr.start(lambda: adb_call_parser, log_stream=self._log_stream)
        ...
        np.testing.assert_almost_equal(expected_extra.get("extra_list"), extras.get("extra_list"))
        np.testing.assert_equal(expected_extra.get("extra_string"), extras.get("extra_string"))
        np.testing.assert_almost_equal(expected_extra.get("extra_float"), extras.get("extra_float"))
        np.testing.assert_equal(expected_extra.get("extra_dict"), extras.get("extra_dict"))
    ...
    def test_get_current_extras_failed_to_parse(self):
        ...
        self.assertIn("extras", timestep.observation)
        extras = timestep.observation["extras"]
        np.testing.assert_almost_equal(extras.get("extra_with_malformed_1"), [[1], [2]])
    ...
    def test_multi_log_regexp(self):
        ...
        task.log_parsing_config.log_regexps.reward.extend(
            ["^[Rr]eward_1: ([-+]?[0-9]*\\.?[0-9]*)$", "^[Rr]eward_2: ([-+]?[0-9]*\\.?[0-9]*)$"]
        )
        task_mgr = task_manager.TaskManager(task=task)
        self._logcat_thread.add_event_listener.side_effect = my_add_ev_listener
        adb_call_parser = mock.create_autospec(adb_call_parser_lib.AdbCallParser)
