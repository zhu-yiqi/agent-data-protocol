...
class TaskManager:
    ...
    def rl_reset(self, observation: dict[str, Any]) -> dm_env.TimeStep:
        ...
        return dm_env.TimeStep(
            step_type=dm_env.StepType.FIRST, reward=0.0, discount=0.0, observation=observation
        )
    ...
    def _start_setup_step_interpreter(self, adb_call_parser: adb_call_parser_lib.AdbCallParser):
    ...
    def _start_dumpsys_thread(self, adb_call_parser: adb_call_parser_lib.AdbCallParser):
        ...
        def _process_extra(extra_name, extra):
            ...
            with self._lock:
                ...
                if extra_name in latest_extras:
                    ...
                    if len(latest_extras[extra_name]) >= self._config.extras_max_buffer_size:
