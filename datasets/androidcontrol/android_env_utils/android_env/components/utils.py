...
def orient_pixels(frame: np.ndarray, orientation: int) -> np.ndarray:
    ...
    match orientation:
        ...
        case _:
            raise ValueError("Orientation must be an integer in [0, 3] but is %r" % orientation)


def convert_int_to_float(
    data: np.ndarray, data_spec: specs.Array, float_type: np_typing.DTypeLike = np.float32
):
