...
from android_env.components import adb_call_parser, adb_controller
...
class AdbCallParserTest(parameterized.TestCase):
    def test_unknown_command(self):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest()
        response = parser.parse(request)
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.UNKNOWN_COMMAND)
    ...
    def test_invalid_timeout(self):
        ...
        response = parser.parse(request)
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.FAILED_PRECONDITION)
    ...
    def test_start_activity_successful(self):
        ...
        adb.execute_command.assert_has_calls(
            [
                mock.call(
                    ["shell", "am", "start", "-S", "-W", "-n", "my.project.SplashActivity", "blah"],
                    timeout=None,
                ),
            ]
        )
    ...
    def test_start_activity_successful_no_force_stop(self):
        ...
        adb.execute_command.assert_has_calls(
            [
                mock.call(
                    ["shell", "am", "start", "", "-W", "-n", "my.project.SplashActivity", "blah"],
                    timeout=None,
                ),
            ]
        )
    ...
    def test_start_activity_error(self):
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.INTERNAL_ERROR)
        self.assertEqual(
            response.error_message, f"start_activity failed with error: {str(command_output)}"
        )
    ...
    def test_press_button_successful(self):
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_with(["shell", "input", "keyevent", "KEYCODE_HOME"], None)
        # BACK.
        request = adb_pb2.AdbRequest()
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_with(["shell", "input", "keyevent", "KEYCODE_BACK"], None)
        # ENTER.
        request = adb_pb2.AdbRequest()
    ...
    def test_start_screen_pinning_package_not_found(self):
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.INTERNAL_ERROR)
        self.assertNotEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "am", "stack", "list"], None)
    ...
    def test_send_broadcast_empty_action(self):
        adb = mock.create_autospec(adb_controller.AdbController)
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(send_broadcast=adb_pb2.AdbRequest.SendBroadcast())
        response = parser.parse(request)
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.FAILED_PRECONDITION)
        self.assertNotEmpty(response.error_message)
    ...
    def test_get_current_activity_no_visible_task(self):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(get_current_activity=adb_pb2.AdbRequest.GetCurrentActivity())
        response = parser.parse(request)
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.INTERNAL_ERROR)
        self.assertNotEmpty(response.error_message)
        adb.execute_command.assert_has_calls(
            [
                mock.call(
                    ["shell", "am", "stack", "list", "|", "grep", "-E", "visible=true"], None
                ),
                mock.call(["shell", "am", "stack", "list"], None),
            ]
        )
    ...
    def test_get_orientation_empty_dumpsys(self):
        ...
        adb.execute_command.return_value = b""
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(get_orientation=adb_pb2.AdbRequest.GetOrientationRequest())
        response = parser.parse(request)
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.INTERNAL_ERROR)
        self.assertNotEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "dumpsys", "input"], None)
    ...
    def test_get_orientation_success(self, orientation: bytes, expected_orientation: int):
        adb = mock.create_autospec(adb_controller.AdbController)
        adb.execute_command.return_value = (
...
"""
    ...
    def test_get_current_activity_no_matches(self):
        ...
        for platform in ["win32", "linux"]:
            with mock.patch.object(sys, "platform", autospec=True, return_value=platform):
                response = parser.parse(request)
                self.assertEqual(response.status, adb_pb2.AdbResponse.Status.INTERNAL_ERROR)
                self.assertNotEmpty(response.error_message)
                adb.execute_command.assert_has_calls(
    ...
    def test_get_current_activity_successful(self):
        ...
        for platform in ["win32", "linux"]:
            with mock.patch.object(sys, "platform", autospec=True, return_value=platform):
                ...
                self.assertEqual(response.get_current_activity.full_activity, "MyAwesomeActivity")
    ...
    def test_push_successful(self):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(
            push=adb_pb2.AdbRequest.Push(content=b"My text.", path="/sdcard/my_file.txt")
        )
    ...
    def test_pull_successful(self, mock_open):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(pull=adb_pb2.AdbRequest.Pull(path="/sdcard/my_file.txt"))
    ...
    def test_input_text_successful(self):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(
            input_text=adb_pb2.AdbRequest.InputText(text="The Greatest Text of All Time")
        )
        response = parser.parse(request)
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
    ...
    @parameterized.named_parameters(
        ("negative_x_and_negative_y", adb_pb2.AdbRequest(tap=adb_pb2.AdbRequest.Tap(x=-1, y=-1))),
        ("negative_x", adb_pb2.AdbRequest(tap=adb_pb2.AdbRequest.Tap(x=-1, y=123))),
        ("negative_y", adb_pb2.AdbRequest(tap=adb_pb2.AdbRequest.Tap(x=456, y=-1))),
    )
    ...
    def test_tap_successful(self):
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "input", "tap", "135", "246"], None)
    ...
    def test_settings_success_reset(self, mode, package_name, expected_arg):
        ...
        request = adb_pb2.AdbRequest.SettingsRequest(
            name_space=adb_pb2.AdbRequest.SettingsRequest.Namespace.GLOBAL,
            reset=adb_pb2.AdbRequest.SettingsRequest.Reset(package_name=package_name, mode=mode),
        )
        request = adb_pb2.AdbRequest(settings=request)
        response = parser.parse(request)
    ...
    def test_settings_success_list(self):
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        self.assertEqual(response.settings.output, b"volume_ring=5\nvolume_system=7")
        adb.execute_command.assert_called_once_with(["shell", "settings", "list", "system"], None)
    ...
    def test_generic_command_timeout(self):
        adb = mock.create_autospec(adb_controller.AdbController)
        args = ["shell", "am", "broadcast", "-n", "receiver", "-a", "action"]
        adb.execute_command.side_effect = subprocess.TimeoutExpired(cmd="cmd", timeout=10)
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            adb_pb2.AdbRequest(
                package_manager=adb_pb2.AdbRequest.PackageManagerRequest(
                    list=adb_pb2.AdbRequest.PackageManagerRequest.List(
                        libraries=adb_pb2.AdbRequest.PackageManagerRequest.List.Libraries()
                    )
                )
            ),
        ),
...
"""
        ...
        self.assertEqual(
            response.package_manager.list.items,
            [
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "pm", "list", "features"], None)
...
"""
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "pm", "list", "libraries"], None)
...
"""
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "pm", "list", "packages"], None)
    ...
    def test_package_manager_clear_no_package_name(self):
        ...
        request = adb_pb2.AdbRequest(
            package_manager=adb_pb2.AdbRequest.PackageManagerRequest(
                clear=adb_pb2.AdbRequest.PackageManagerRequest.Clear(package_name="")
            )
        )
        response = parser.parse(request)
    ...
    def test_package_manager_clear_successful_no_user_id(self):
        ...
        request = adb_pb2.AdbRequest(
            package_manager=adb_pb2.AdbRequest.PackageManagerRequest(
                clear=adb_pb2.AdbRequest.PackageManagerRequest.Clear(package_name="my.package")
            )
        )
        response = parser.parse(request)

        self.assertEqual(response.package_manager.output, b"""Some successful message.""")
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "pm", "clear", "my.package"], None)
    ...
    def test_dumpsys_empty_request(self):
        ...
        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "dumpsys"], timeout=None)

    @parameterized.named_parameters(
        (
            "negative_timeout_sec",
            adb_pb2.AdbRequest(dumpsys=adb_pb2.AdbRequest.DumpsysRequest(timeout_sec=-1)),
        ),
        (
            "negative_timeout_ms",
            adb_pb2.AdbRequest(dumpsys=adb_pb2.AdbRequest.DumpsysRequest(timeout_ms=-2)),
        ),
    )
    ...
    @parameterized.named_parameters(
        ...
        (
            "priority_high",
            adb_pb2.AdbRequest.DumpsysRequest.PriorityLevel.HIGH,
    ...
    def test_dumpsys_priority_timeout_successful(self, priority, expected):
        ...
        adb.execute_command.return_value = b"whatever"
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(dumpsys=adb_pb2.AdbRequest.DumpsysRequest(priority=priority))
    ...
    @parameterized.named_parameters(
        ...
        (
            "arbitrary_args",
            adb_pb2.AdbRequest.DumpsysRequest(list_only=True, args=["myoption", "anotheroption"]),
        ),
        (
            "skip_usb",
            adb_pb2.AdbRequest.DumpsysRequest(list_only=True, skip_services=["usb"]),
        ),
    )
    ...
    def test_dumpsys_list_only_success(self):
        ...
        adb.execute_command.return_value = b"whatever"
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(dumpsys=adb_pb2.AdbRequest.DumpsysRequest(list_only=True))

        response = parser.parse(request)

        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "dumpsys", "-l"], timeout=None)
    ...
    def test_dumpsys_skip_services(self):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(
            dumpsys=adb_pb2.AdbRequest.DumpsysRequest(skip_services=["window", "usb"])
        )
    ...
    def test_dumpsys_single_service(self):
        ...
        adb.execute_command.return_value = b"whatever"
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(dumpsys=adb_pb2.AdbRequest.DumpsysRequest(service="window"))

        response = parser.parse(request)

        self.assertEqual(response.status, adb_pb2.AdbResponse.Status.OK)
        self.assertEmpty(response.error_message)
        adb.execute_command.assert_called_once_with(["shell", "dumpsys", "window"], timeout=None)
    ...
    def test_dumpsys_single_service_with_args(self):
        ...
        parser = adb_call_parser.AdbCallParser(adb, tmp_dir=self.create_tempdir().full_path)
        request = adb_pb2.AdbRequest(
            dumpsys=adb_pb2.AdbRequest.DumpsysRequest(service="window", args=["arg1", "arg2"])
        )
