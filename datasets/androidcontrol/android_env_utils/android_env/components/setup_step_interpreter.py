...
from android_env.components import app_screen_checker, errors
    ...
    def _process_step_command(self, step_cmd: task_pb2.SetupStep) -> None:
        ...
        while num_tries < max_retries:
            ...
            try:
                ...
                logging.exception(
                    "ADB call [%r] has failed. Try %d of %d.",
                    step_cmd.adb_request,
                    num_tries,
                    max_retries,
                )
                ...
                logging.exception(
                    "Failed to wait for app screen. Try %d of %d.", num_tries, max_retries
                )
                ...
                logging.exception(
                    "Package [%r] not installed. Try %d of %d.",
                    success_condition.check_install.package_name,
                    num_tries,
                    max_retries,
                )

        raise errors.StepCommandError(f"Step failed: [{step_cmd}]") from latest_error
