...
class AdbCallParser:
    ...
    def _execute_command(
        ...
        command_output = b""
        try:
            command_output = self._adb_controller.execute_command(command_args, timeout=timeout)
    ...
    def _fetch_current_task_id(self, full_activity_name: str, timeout: float | None = None) -> int:
        ...
        for line in lines:
            ...
            if full_activity_name not in {base_activity, top_activity}:
                logging.info(
                    "Full activity %s was not found in current line %s", full_activity_name, line
                )
                continue
    ...
    def _start_screen_pinning(
        ...
        if not full_activity:
            response.status = adb_pb2.AdbResponse.Status.FAILED_PRECONDITION
            response.error_message = "`start_screen_pinning.full_activity` cannot be empty."
            return response
    ...
    def _send_broadcast(
        ...
        response, _ = self._execute_command(
            ["shell", "am", "broadcast", "-a", send_broadcast.action] + component_args,
            timeout=timeout,
        )
    ...
    def _install_apk(
        ...
        response, _ = self._execute_command(["install", "-r", "-t", "-g", fpath], timeout=timeout)
        return response
    ...
    def _press_button(
        ...
        if button not in _BUTTON_TO_KEYCODE:
            return adb_pb2.AdbResponse(
                status=adb_pb2.AdbResponse.Status.FAILED_PRECONDITION,
                error_message=(
                    "PressButton.button must be one of "
    ...
    def _handle_uninstall_package(
        ...
        if not package_name:
            response.status = adb_pb2.AdbResponse.Status.FAILED_PRECONDITION
            response.error_message = "`uninstall_package.package_name` cannot be empty."
            return response
    ...
    def _get_current_activity(
        ...
        response, visible_task = self._execute_command(
            ["shell", "am", "stack", "list", "|", "grep", "-E", "visible=true"], timeout=timeout
        )
        ...
        if not visible_task:
            _, am_stack_list = self._execute_command(
                ["shell", "am", "stack", "list"], timeout=timeout
            )
            response.status = adb_pb2.AdbResponse.Status.INTERNAL_ERROR
    ...
    def _get_orientation(
        ...
        response = self._handle_dumpsys(
            adb_pb2.AdbRequest(dumpsys=adb_pb2.AdbRequest.DumpsysRequest(service="input")),
            timeout=timeout,
        )
    ...
    def _pull(
        ...
        with tempfile.NamedTemporaryFile(dir=self._tmp_dir, delete=False) as f:
            ...
            response, _ = self._execute_command(["pull", path, fname], timeout=timeout)
    ...
    def _input_text(
        ...
        response, _ = self._execute_command(["shell", "input", "text", text], timeout=timeout)
        return response
    ...
    def _tap(
        ...
        if x < 0 or y < 0:
            return adb_pb2.AdbResponse(
                status=adb_pb2.AdbResponse.Status.FAILED_PRECONDITION,
                error_message=(f"Tap coordinates must be non-negative. Got: {request.tap}."),
            )
    ...
    def _handle_settings(
        ...
        if request.name_space == adb_pb2.AdbRequest.SettingsRequest.Namespace.UNKNOWN:
            response.status = adb_pb2.AdbResponse.Status.FAILED_PRECONDITION
            response.error_message = f"Unknown SettingsRequest.name_space. Got: {request}."
            return response

        namespace = adb_pb2.AdbRequest.SettingsRequest.Namespace.Name(request.name_space).lower()
        ...
        match request.WhichOneof("verb"):
            case "get":
                ...
                if not get.key:
                    response.status = adb_pb2.AdbResponse.Status.FAILED_PRECONDITION
                    response.error_message = f"Empty SettingsRequest.get.key. Got: {request}."
                    return response
                response, command_output = self._execute_command(
            ...
            case "reset":
                ...
                if (
                    ...
                    and reset.mode == adb_pb2.AdbRequest.SettingsRequest.Reset.Mode.UNKNOWN
                ):
                    response.status = adb_pb2.AdbResponse.Status.FAILED_PRECONDITION
                    response.error_message = (
                ...
                mode = adb_pb2.AdbRequest.SettingsRequest.Reset.Mode.Name(reset.mode).lower()
            ...
            case _:
                response.status = adb_pb2.AdbResponse.Status.FAILED_PRECONDITION
                response.error_message = f"Unknown SettingsRequest.verb. Got: {request}."
    ...
    def _handle_generic(
        ...
        response, command_output = self._execute_command(list(request.generic.args), timeout)
    ...
    def _handle_dumpsys(
        ...
        if request.priority != adb_pb2.AdbRequest.DumpsysRequest.PriorityLevel.UNSET:
            cmd.append("--priority")
            cmd.append(adb_pb2.AdbRequest.DumpsysRequest.PriorityLevel.Name(request.priority))
        ...
        response, response.dumpsys.output = self._execute_command(cmd, timeout=timeout)
