...
from android_env.proto.a11y import a11y_pb2, android_accessibility_forest_pb2
...
class A11yServicerTest(parameterized.TestCase, IsolatedAsyncioTestCase):
                ...
                _aiter(
                    [
                        a11y_pb2.ClientToServer(
                            event=a11y_pb2.EventRequest(event=single_item_dict_with_special_chars())
                        ),
    ...
    def test_servicer_sendevent(self):
        ...
        servicer = a11y_servicer.A11yServicer()
        servicer.resume()
        response = servicer.SendEvent(a11y_pb2.EventRequest(event=empty_dict()), mock_context)
        self.assertEqual(response.error, "")
    ...
    async def test_servicer_bidi_events(self):
        ...
        responses = [
            ...
            async for x in servicer.Bidi(
                _aiter(
                    [
                        a11y_pb2.ClientToServer(event=a11y_pb2.EventRequest(event=empty_dict())),
                        a11y_pb2.ClientToServer(
                            event=a11y_pb2.EventRequest(event=single_item_dict_with_special_chars())
                        ),
                    ]
