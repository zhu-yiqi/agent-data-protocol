...
from android_env.components import adb_call_parser, errors, setup_step_interpreter
from android_env.proto import adb_pb2, task_pb2
...
class SetupStepInterpreterTest(absltest.TestCase):
    def setUp(self):
        super().setUp()
        self._parser = mock.create_autospec(adb_call_parser.AdbCallParser, instance=True)
    ...
    def test_empty_setup_steps(self):
        ...
        """
        interpreter = setup_step_interpreter.SetupStepInterpreter(adb_call_parser=self._parser)
    ...
    def test_invalid_setup_step(self):
        interpreter = setup_step_interpreter.SetupStepInterpreter(adb_call_parser=self._parser)
        # Empty setup steps should be ignored.
        self.assertRaises(AssertionError, interpreter.interpret, [task_pb2.SetupStep()])

    def test_adb_install_apk_filesystem(self):
        self._parser.parse.return_value = adb_pb2.AdbResponse(status=adb_pb2.AdbResponse.Status.OK)
        interpreter = setup_step_interpreter.SetupStepInterpreter(adb_call_parser=self._parser)
        interpreter.interpret(
            [
                _to_proto(
...
adb_request: { force_stop: { package_name: "my.app.Activity" } }""",
        ...
        self._parser.parse.assert_called_once_with(
            adb_pb2.AdbRequest(
                force_stop=adb_pb2.AdbRequest.ForceStop(package_name="my.app.Activity")
            )
        )
...
adb_request: {
  tap: {
    ...
    def test_adb_press_button(self):
        ...
        interpreter = setup_step_interpreter.SetupStepInterpreter(adb_call_parser=self._parser)
        interpreter.interpret(
            [_to_proto(task_pb2.SetupStep, """ adb_request: { press_button: { button: HOME } }""")]
        )
        self._parser.parse.assert_called_once_with(
            adb_pb2.AdbRequest(
        ...
        interpreter.interpret(
            [_to_proto(task_pb2.SetupStep, """ adb_request: { press_button: { button: BACK } }""")]
        )
        self._parser.parse.assert_called_once_with(
            adb_pb2.AdbRequest(
                press_button=adb_pb2.AdbRequest.PressButton(
...
adb_request: {
  start_screen_pinning: {
    ...
    def test_time_sleep(self, mock_sleep):
        interpreter = setup_step_interpreter.SetupStepInterpreter(adb_call_parser=self._parser)
        interpreter.interpret([_to_proto(task_pb2.SetupStep, """sleep: { time_sec: 0.875 }""")])
    ...
    def test_wait_for_app_screen_empty_activity(self, unused_mock_sleep):
        ...
        with self.assertRaises(errors.StepCommandError):
            interpreter.interpret(
                [_to_proto(task_pb2.SetupStep, """success_condition: {wait_for_app_screen: { }}""")]
            )
