...
def base_action_spec(
    ...
    action_spec = {
        "action_type": specs.DiscreteArray(num_values=num_actions, name="action_type"),
    ...
    for i in range(2, num_fingers + 1):
        action_spec.update(
            {
                f"action_type_{i}": specs.DiscreteArray(
                    num_values=len(action_type.ActionType), name=f"action_type_{i}"
                ),
                f"touch_position_{i}": specs.BoundedArray(
                    shape=(2,),
                    dtype=np.float32,
                    minimum=[0.0, 0.0],
                    maximum=[1.0, 1.0],
    ...
    if enable_key_events:
        action_spec["keycode"] = specs.DiscreteArray(num_values=(1 << 16) - 1, name="keycode")
...
def base_observation_spec(height: int, width: int) -> dict[str, specs.Array]:
    ...
    return {
        "pixels": specs.BoundedArray(
            shape=(height, width, 3), dtype=np.uint8, name="pixels", minimum=0, maximum=255
        ),
        "timedelta": specs.Array(shape=(), dtype=np.int64, name="timedelta"),
        "orientation": specs.BoundedArray(
            shape=np.array([4]), dtype=np.uint8, name="orientation", minimum=0, maximum=1
        ),
    }
